...
def main():
    ...
    eval_files_itodd_ours = \
        [
            ...
            'results/ITODD/ours/SARR/sarr-object_itodd-test.csv',
            'results/ITODD/ours/SARR/sarr-symmetry_itodd-test.csv',
            'results/ITODD/ours/SARR/sarr-dataset_itodd-test.csv',
            'results/ITODD/ours/SARR/sarr-datasetast_itodd-test.csv',
            'results/ITODD/ours/SARR/sarr-object-rgb_itodd-test.csv',
            'results/ITODD/ours/SARR/sarr-symmetry-rgb_itodd-test.csv',
            'results/ITODD/ours/SARR/sarr-dataset-rgb_itodd-test.csv',
    ...
    for e_file in eval_file_full_paths:
        print('------------------------------------------------')
        res_file = e_file.replace(e_file.split('//')[-1], f'{"T-LESS" if "tless" in e_file else "ITODD"}_erot_results_{"ours" if "ours" in e_file else "others"}.txt')
        name = e_file.split('/')[-1]
        print(name)
        ...
        for task in ['siso', 'vivo']:
            ...
            ar = 100 * np.round(np.array(recalls).mean(), decimals=3)
            res_string = f"{task}: AR: {ar:.1f}"
            print(res_string)
            with open(res_file, 'a') as f:
                f.write(res_string + '\n')
        with open(res_file, 'a') as f:
